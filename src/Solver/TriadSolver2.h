#ifndef TRIAD_SOLVER2
#define TRIAD_SOLVER2
...
// Staggering indices are a runtime variable
...
private:
    ...
    // const StaggerIndexing2 m_staggerIndexingU, m_staggerIndexingV, m_staggerIndexingW;
...
public:
    TriadSolver2( FieldData<Tensor3D> &fields,
    ...
    { 
        ...
        assert( (void("Invalid U momentum staggering"), (Ustag == TC::e) || (Ustag == TC::w) || (Ustag == TC::p) ) );
        assert( (void("Invalid V momentum staggering"), (Vstag == TC::n) || (Vstag == TC::s) || (Vstag == TC::p) ) );
        assert( (void("Invalid V momentum staggering"), (Wstag == TC::t) || (Wstag == TC::b) || (Wstag == TC::p) ) );
    ...
    inline void UpdateTriad( const intType i, 
                             ...
                             const StaggerIndexing2 &staggerIndexingU,
                             const StaggerIndexing2 &staggerIndexingV,
                             const StaggerIndexing2 &staggerIndexingW )
    {
        using namespace FVT;
        using enum Axis::ENUMDATA;
        using enum TransportCoefficients::ENUMDATA;

        const StaggerIndexing2::ContinuityVelocity &sCU = staggerIndexingU.continuityVelocity, 
                                                   &sCV = staggerIndexingV.continuityVelocity, 
                                                   &sCW = staggerIndexingW.continuityVelocity;
        const StaggerIndexing2::MomentumPressure   &sUP = staggerIndexingU.momentumPressure, 
                                                   &sVP = staggerIndexingV.momentumPressure, 
                                                   &sWP = staggerIndexingW.momentumPressure;
                             ...
                             const StaggerIndexing2 &staggerIndexingW )
    {
        ...
        UpdateMolecule( i, j, k, bU, bV, bW, bP, staggerIndexingU, staggerIndexingV, staggerIndexingW );
                                 ...
                                 const StaggerIndexing2 &staggerIndexingW )
    {
        ...
        UpdateMoleculeFull( i, j, k, bUS, bUC, bVS, bVC, bWS, bWC, bP, staggerIndexingU, staggerIndexingV, staggerIndexingW );
