...
std::array< fVector3, 8> GetBlockVertices( const SolidBlockData &blockData )
{
...
Polyhedron MakeBlockPolyhedron( const SolidBlockData &solidBlockData )
{
    using enum Axis::ENUMDATA;
...
void AddBlocks( std::vector< Polyhedron > &geometryPolyhedra,
                const std::vector< SolidBlockData > &solidBlocks )
{
    for ( const SolidBlockData &solidBlockData : solidBlocks ) {
...
Polyhedron MakeSpherePolyhedron( const SolidSphereData &solidSphereData )
{
    using enum Axis::ENUMDATA;
...
void AddSpheres( std::vector< Polyhedron > &geometryPolyhedra,
                 const std::vector< SolidSphereData > &solidSpheres )
{
    for ( const SolidSphereData &solidSphereData : solidSpheres ) {
...
void AddSTLFiles( std::vector< Polyhedron > &geometryPolyhedra,
                  const std::vector<STLGeometryData> &stlGeometries,
                  const AxisTransformationMap &axisTransformation )
{

    for ( const STLGeometryData &stlGeometryData : stlGeometries ) {
