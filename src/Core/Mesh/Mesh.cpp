...
std::vector<floatType> CalculateGrowthRates(const std::vector<MeshSegment> &meshSegments)
{
...
void CalculateCellLengths(Tensor1D &cellLengths, 
                          const std::vector<MeshSegment> &meshSegments, 
...
intType TotalCells(const std::vector<MeshSegment> &meshSegments)
{
...
// Constructor, creates the mesh from user mesh segments
Mesh::Mesh(const EnumVector< Axis, std::vector< MeshSegment > > &meshSegments ) :
    Mesh( iArray3({ TotalCells(meshSegments[Axis::X]),  TotalCells(meshSegments[Axis::Y]), TotalCells(meshSegments[Axis::Z])}) )
    ...
    { 
        ...
        EnumFor<Axis>( [&] (Axis::ENUMDATA axis) {
            ...
            growthRates[axis] = CalculateGrowthRates(meshSegments[axis]);

            CalculateCellLengths(cellLengths[axis], meshSegments[axis], growthRates[axis]);
            cellLengthsInv[axis] = cellLengths[axis].inverse();

            CalculateCellCenters(cellCenters[axis], cellLengths[axis], meshSegments[axis].front().startCoordinate);
            CalculateCellCenterDiffInv(cellCenterDiffInv[axis], cellLengths[axis], cellCenters[axis]);

            CalculateCellFaces(cellFaces[axis], cellLengths[axis], meshSegments[axis].front().startCoordinate);
...
Mesh::Mesh( const EnumVector<Axis, Tensor1D> &cellFacesArg ) :
    Mesh( iArray3({ cellFacesArg[Axis::X].size()-1, cellFacesArg[Axis::Y].size()-1, cellFacesArg[Axis::Z].size()-1}) )
    { 
        EnumFor<Axis>( [&] (Axis::ENUMDATA axis) {
...
Mesh CreateMesh( const EnumVector< Axis, std::vector< MeshSegment > > &meshSegments,
                 const AxisTransformationMap &atm )
{
    using enum Axis::ENUMDATA;

    Mesh mesh(meshSegments);
