...
void SetPressureMomentum(FVCoefficients &fvCoeffs, const Mesh &mesh, const InputData::BoundaryConditionData &boundaryConditions)
{
    ...
    for (iterType i = 0; i != iEnd; i++) {
        fvCoeffs.aup[p](i) = 1 - mesh.interpFactors[X](i) - mesh.interpFactors[X](i-1);
        fvCoeffs.aup[e](i) = mesh.interpFactors[X](i);
        fvCoeffs.aup[w](i) = - ( 1 - mesh.interpFactors[X](i-1) ); 
    }

    // V momentum 
    for (iterType j = 0; j != jEnd; j++) {
        fvCoeffs.avp[p](j) = 1 - mesh.interpFactors[Y](j) - mesh.interpFactors[Y](j-1);
        fvCoeffs.avp[n](j) = mesh.interpFactors[Y](j);
        fvCoeffs.avp[s](j) = - ( 1 - mesh.interpFactors[Y](j-1) ); 
    }

    // W momentum 
    for (iterType k = 0; k != kEnd; k++) {
        fvCoeffs.awp[p](k) = 1 - mesh.interpFactors[Z](k) - mesh.interpFactors[Z](k-1);
        fvCoeffs.awp[t](k) = mesh.interpFactors[Z](k);
        fvCoeffs.awp[b](k) = - ( 1 - mesh.interpFactors[Z](k-1) ); 
    }
