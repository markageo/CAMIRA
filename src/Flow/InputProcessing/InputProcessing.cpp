...
namespace
{
    ...
    void ValidateSegmentBounds(const std::vector<MeshSegment> &meshSegment, 
    ...
    void ReadGrid( std::vector<MeshSegment> &meshSegments,
                        ...
                        const std::pair<const std::string, pt::ptree> &solidObject )
    {
        ...
        inputData.geometryData.solidBlocks.push_back( tempBlockData );
    }
    ...
    void ReadSphereData( InputData &inputData,
                        const std::pair<const std::string, pt::ptree> &solidObject )
    {
        SolidSphereData tempSphereData;
        ...
        inputData.geometryData.solidSpheres.push_back( tempSphereData );
    }
                                   ...
                                   const std::pair<const std::string, pt::ptree> &solidObject )
    {
        ...
        if ( valueString == "STL" ) {
            ...
            inputData.geometryData.stlGeometries.emplace_back();

            // filename
            inputData.geometryData.stlGeometries.back().filename = solidObject.second.get< std::string >( "filename" );
            IOTOOLS::PrependRelativePath( inputData.geometryData.stlGeometries.back().filename, inputData.inputFileDirectory );
            ...
            if ( rotationOptional ) {
                EnumFor<Axis>( [&] (Axis::ENUMDATA axis) {
                    inputData.geometryData.stlGeometries.back().rotation(axis) = rotationOptional.get()[axis];
                } );
            } else {
                inputData.geometryData.stlGeometries.back().rotation = {0, 0, 0};
            }
    ...
    void VerifyOutputFiles( InputData &inputData ) 
    {
        ...
        IOTOOLS::CreatePathIfDoesNotExist( inputData.residualHistoryFilename );
        IOTOOLS::CreatePathIfDoesNotExist( inputData.profilingFilename );
        IOTOOLS::CreatePathIfDoesNotExist( inputData.geometryOutputFilename );
        for ( const auto &probe : inputData.probes ) {
            IOTOOLS::CreatePathIfDoesNotExist( probe.filename );
        }
        IOTOOLS::CreatePathIfDoesNotExist( inputData.forceCalculatorFilename );
        IOTOOLS::CreatePathIfDoesNotExist( inputData.yPlusCalculatorFilename );
    }
