    ...
    for ( intType k = 0; k != mesh.nCells(Z); k++ ) {
        for ( intType j = 0; j != mesh.nCells(Y); j++ ) {
            for ( intType i = 0; i != mesh.nCells(X); i++ ) {
                ...
                const floatType Sx = -nu*(-2*std::pow(M_PI, 2)*(std::pow(std::sin(M_PI*x), 2) - std::pow(std::cos(M_PI*x), 2))*std::sin(M_PI*y)*std::sin(M_PI*z)*std::cos(M_PI*y)*std::cos(M_PI*z) - 8*std::pow(M_PI, 2)*std::pow(std::sin(M_PI*x), 2)*std::sin(M_PI*y)*std::sin(M_PI*z)*std::cos(M_PI*y)*std::cos(M_PI*z)) - 2*(-M_PI*std::pow(std::sin(M_PI*x), 2)*std::sin(M_PI*y)*std::pow(std::sin(M_PI*z), 2)*std::cos(M_PI*y) + M_PI*std::pow(std::sin(M_PI*x), 2)*std::sin(M_PI*y)*std::cos(M_PI*y)*std::pow(std::cos(M_PI*z), 2))*std::sin(M_PI*x)*std::sin(M_PI*y)*std::pow(std::sin(M_PI*z), 2)*std::cos(M_PI*x)*std::cos(M_PI*y) + (-M_PI*std::pow(std::sin(M_PI*x), 2)*std::pow(std::sin(M_PI*y), 2)*std::sin(M_PI*z)*std::cos(M_PI*z) + M_PI*std::pow(std::sin(M_PI*x), 2)*std::sin(M_PI*z)*std::pow(std::cos(M_PI*y), 2)*std::cos(M_PI*z))*std::sin(M_PI*x)*std::pow(std::sin(M_PI*y), 2)*std::sin(M_PI*z)*std::cos(M_PI*x)*std::cos(M_PI*z) + 2*M_PI*std::pow(std::sin(M_PI*x), 3)*std::pow(std::sin(M_PI*y), 2)*std::pow(std::sin(M_PI*z), 2)*std::cos(M_PI*x)*std::pow(std::cos(M_PI*y), 2)*std::pow(std::cos(M_PI*z), 2) + M_PI*std::sin(M_PI*y)*std::sin(M_PI*z)*std::cos(M_PI*x)/rho;

                const floatType Sy = -nu*(-2*std::pow(M_PI, 2)*(std::pow(std::sin(M_PI*y), 2) - std::pow(std::cos(M_PI*y), 2))*std::sin(M_PI*x)*std::sin(M_PI*z)*std::cos(M_PI*x)*std::cos(M_PI*z) - 8*std::pow(M_PI, 2)*std::sin(M_PI*x)*std::pow(std::sin(M_PI*y), 2)*std::sin(M_PI*z)*std::cos(M_PI*x)*std::cos(M_PI*z)) - 2*(-M_PI*std::sin(M_PI*x)*std::pow(std::sin(M_PI*y), 2)*std::pow(std::sin(M_PI*z), 2)*std::cos(M_PI*x) + M_PI*std::sin(M_PI*x)*std::pow(std::sin(M_PI*y), 2)*std::cos(M_PI*x)*std::pow(std::cos(M_PI*z), 2))*std::sin(M_PI*x)*std::sin(M_PI*y)*std::pow(std::sin(M_PI*z), 2)*std::cos(M_PI*x)*std::cos(M_PI*y) + (-M_PI*std::pow(std::sin(M_PI*x), 2)*std::pow(std::sin(M_PI*y), 2)*std::sin(M_PI*z)*std::cos(M_PI*z) + M_PI*std::pow(std::sin(M_PI*y), 2)*std::sin(M_PI*z)*std::pow(std::cos(M_PI*x), 2)*std::cos(M_PI*z))*std::pow(std::sin(M_PI*x), 2)*std::sin(M_PI*y)*std::sin(M_PI*z)*std::cos(M_PI*y)*std::cos(M_PI*z) + 2*M_PI*std::pow(std::sin(M_PI*x), 2)*std::pow(std::sin(M_PI*y), 3)*std::pow(std::sin(M_PI*z), 2)*std::pow(std::cos(M_PI*x), 2)*std::cos(M_PI*y)*std::pow(std::cos(M_PI*z), 2) + M_PI*std::sin(M_PI*x)*std::sin(M_PI*z)*std::cos(M_PI*y)/rho;

                const floatType Sz = -nu*(4*std::pow(M_PI, 2)*(std::pow(std::sin(M_PI*z), 2) - std::pow(std::cos(M_PI*z), 2))*std::sin(M_PI*x)*std::sin(M_PI*y)*std::cos(M_PI*x)*std::cos(M_PI*y) + 16*std::pow(M_PI, 2)*std::sin(M_PI*x)*std::sin(M_PI*y)*std::pow(std::sin(M_PI*z), 2)*std::cos(M_PI*x)*std::cos(M_PI*y)) + (2*M_PI*std::sin(M_PI*x)*std::pow(std::sin(M_PI*y), 2)*std::pow(std::sin(M_PI*z), 2)*std::cos(M_PI*x) - 2*M_PI*std::sin(M_PI*x)*std::pow(std::sin(M_PI*z), 2)*std::cos(M_PI*x)*std::pow(std::cos(M_PI*y), 2))*std::sin(M_PI*x)*std::pow(std::sin(M_PI*y), 2)*std::sin(M_PI*z)*std::cos(M_PI*x)*std::cos(M_PI*z) + (2*M_PI*std::pow(std::sin(M_PI*x), 2)*std::sin(M_PI*y)*std::pow(std::sin(M_PI*z), 2)*std::cos(M_PI*y) - 2*M_PI*std::sin(M_PI*y)*std::pow(std::sin(M_PI*z), 2)*std::pow(std::cos(M_PI*x), 2)*std::cos(M_PI*y))*std::pow(std::sin(M_PI*x), 2)*std::sin(M_PI*y)*std::sin(M_PI*z)*std::cos(M_PI*y)*std::cos(M_PI*z) + 8*M_PI*std::pow(std::sin(M_PI*x), 2)*std::pow(std::sin(M_PI*y), 2)*std::pow(std::sin(M_PI*z), 3)*std::pow(std::cos(M_PI*x), 2)*std::pow(std::cos(M_PI*y), 2)*std::cos(M_PI*z) + M_PI*std::sin(M_PI*x)*std::sin(M_PI*y)*std::cos(M_PI*z)/rho;
